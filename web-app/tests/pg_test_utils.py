...
def truncate_app_tables() -> None:
    ...
    try:
        with engine.begin() as connection:
            connection.execute(text("TRUNCATE TABLE users, sessions, settings, subtitle_replacements, bgm_tracks, voice_profiles, generation_tasks, templates, organizations RESTART IDENTITY CASCADE"))
    finally:
        engine.dispose()
    dispose_engines()
